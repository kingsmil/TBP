...
def create_case(profile_text: str) -> dict[str, Any]:
    ...
    case: dict[str, Any] = {
        ...
        "conversation": [],
        "status": "running",
        # set during search phase — used to continue refinement
        "search_prefs": {},
        "candidate_ids": [],   # block_ids shortlisted after search
    }
    _cases[case_id] = case
    return case
...
def set_search_state(case_id: str, prefs: dict[str, Any], candidate_ids: list[int]) -> None:
    if case_id in _cases:
        _cases[case_id]["search_prefs"] = prefs
        _cases[case_id]["candidate_ids"] = candidate_ids
